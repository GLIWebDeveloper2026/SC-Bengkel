...
1. **Line Item Classifier (Baris Nota Tidak Seragam):**
   ...
   - `inventory`: Memotong stok desimal (`stock_qty` pada `parts`), tanpa komisi.
...
2. **Multi-Mekanik 1 Work Order (Kejadian C / Pak Sarno & Junior Mechanic):**
   - Penugasan `mechanic_id` berada di level baris pekerjaan (`work_order_items`). 1 Motor dapat dikerjakan bersama oleh Pak Sarno (Kelistrikan) dan Junior Mechanic dengan komisi terpisah secara eksak.

3. **Konversi Satuan & Stok Desimal (Oli 0,8L dari Drum 30L / Kejadian B):**
...
5. **Bulk Payment Matrix (Pemilik Rental 4 Nota / Kejadian F):**
...
4. **Menjalankan Automated Test Suite (5 Test Cases):**
   ```bash
   php artisan test
   ```
...
- `app/Http/Controllers/` — Application Controllers (`WorkOrderController`, `PaymentController`, `WarrantyController`, `ReportController`, `DashboardController`)
- `database/migrations/` — Migration Schema Laravel Engine
- `database/seeders/` — Seeder Mandatory (Pak Hendra, Mbak Rina, Pak Sarno, Pemilik Rental 4 Motor, Oli Drum 30L)
- `resources/views/` — Views UI responsive dark glassmorphic (`layouts/app.blade.php`, `work_orders/`, `invoices/`, `payments/`, `warranty/`, `reports/`)
- `tests/Feature/` — Test Suite 5 Skenario Studi Kasus (`BengkelJayaMotorTest.php`)
